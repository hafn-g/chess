...
public class BoardRenderer {
    ...
    public void drawBoard(Graphics2D g2d) {
        for (int row = 0; row < boardPanel.getMetrics().getRows(); row++) {
            for (int col = 0; col < boardPanel.getMetrics().getCols(); col++) {
                Cell cell = boardPanel.getState().getCell(row, col);
                Piece piece = boardPanel.getState().getPiece(cell);

                int x = boardPanel.getMetrics().getBoardX() + col * boardPanel.getMetrics().getCellSize();
                int y = boardPanel.getMetrics().getBoardY() + row * boardPanel.getMetrics().getCellSize();

                g2d.setColor(getDrawBoardColor(cell));
                g2d.fillRect(x, y, boardPanel.getMetrics().getCellSize(), boardPanel.getMetrics().getCellSize());

                // draw piece
                if (piece != null) {
                    g2d.drawImage(ImageCache.get(piece), x, y, boardPanel.getMetrics().getCellSize(), boardPanel.getMetrics().getCellSize(), null);
                }

                g2d.setColor(Color.BLACK);
                g2d.drawRect(x, y, boardPanel.getMetrics().getCellSize(), boardPanel.getMetrics().getCellSize());
            }
        }
    }
    ...
    public void drawCoords(Graphics2D g2d) {
        g2d.setColor(Color.BLACK);
        g2d.setFont(new Font("Arial", Font.BOLD, boardPanel.getMetrics().getCellSize() / 5));
    ...
    private void drawLetters(Graphics2D g2d, FontMetrics fm) {
        for (int col = 0; col < boardPanel.getMetrics().getCols(); col++) {
            ...
            int x = boardPanel.getMetrics().getBoardX() + col * boardPanel.getMetrics().getCellSize() + (boardPanel.getMetrics().getCellSize() - textWidth) / 2;
            int y = boardPanel.getMetrics().getBoardY() + boardPanel.getMetrics().getSide() + fm.getAscent();
    ...
    private void drawNumbers(Graphics2D g2d, FontMetrics fm) {
        for (int row = 0; row < boardPanel.getMetrics().getRows(); row++) {
            String number = String.valueOf(boardPanel.getMetrics().getRows() - row);

            int textHeight = fm.getAscent();
            int x = boardPanel.getMetrics().getPadding() / 3;
            int y = boardPanel.getMetrics().getBoardY() + row * boardPanel.getMetrics().getCellSize() + (boardPanel.getMetrics().getCellSize() + textHeight) / 2;
    ...
    public void updateHover(int mouseX, int mouseY, int width, int height) {
        ...
        int cellSize = side / boardPanel.getMetrics().getRows();
        ...
        if (row >= 0 && row < boardPanel.getMetrics().getRows() && col >= 0 && col < boardPanel.getMetrics().getCols()) {
    ...
    public void clearSelection() {
        ...
        boardPanel.getState().setClickedCell(null);
    }
