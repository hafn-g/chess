...
@Slf4j
public class ChessMultiplayerFrame extends JFrame {
    private JPanel mainPanel;
    @Getter
    private BoardPanel boardPanel;

    public ChessMultiplayerFrame(GameConfig config) {
        initUI(config);
    }

    private void initUI(GameConfig config) {
        setTitle("Chess Game | Multiplayer | Your color " + config.getMyColor());
    ...
    private BoardPanel initBoard(GameConfig config) {
        ...
        BoardPanel boardPanel = new BoardPanel(config, config.getMyColor());
